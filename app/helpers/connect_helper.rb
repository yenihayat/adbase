module ConnectHelper
  ...
  def ad_content(content_type, zone_uuid, ad_target_url, ad_id, ad_file_name, ad_width, ad_height)
    ...
    if content_type.include?('image')
      ...
      content += "target_url.href = 'http://adbase.local/out/#{ad_id}';\n"
      content += "var ad_content = document.createElement('img');\n"
      content += "ad_content.setAttribute('class', 'ad');\n"
      content += "ad_content.src = 'http://adbase.local/system/ads/#{partitionize(ad_id)}/#{ad_file_name}';\n"
      ...
      content += "ad_content.setAttribute('data', 'http://adbase.local/system/ads/#{partitionize(ad_id)}/#{ad_file_name}');\n"
      content += "var param1 = document.createElement('param');\n"
      content += "param1.setAttribute('name', 'movie');\n"
      content += "param1.setAttribute('value', 'http://adbase.local/system/ads/#{partitionize(ad_id)}/#{ad_file_name}');\n"
